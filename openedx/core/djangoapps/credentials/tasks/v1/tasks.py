...
def backfill_date_for_all_course_runs():
    """
    This task is responsible for enqueueing subtasks responsible for transmitting Certificate Available Date (CAD) data
    to the Credentials IDA. This task is used to backfill missing CADs, or for correcting potentially incorrect CAD
    data already present in Credentials.

    This task enqueues `update_certificate_available_date_on_course_update` subtasks so that we don't have to
    duplicate the "what date to send" and REST API request logic in multiple places.
    """
